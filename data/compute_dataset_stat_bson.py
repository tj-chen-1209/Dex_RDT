...
def process_bson_dataset(vla_dataset):
    """
    处理BSON数据集，计算state的统计信息
    
    Args:
        vla_dataset: BsonDexDataset实例
        
    Returns:
        dict: 包含mean, std, min, max的统计信息
    """
    EPS = 1e-8
    episode_cnt = 0
    state_sum = 0
    ...
    print(f"Processing {len(vla_dataset)} episodes...")
    
    for i in tqdm(range(len(vla_dataset))):
        # 使用state_only=True获取完整轨迹
        episode = vla_dataset.get_item(index=i, state_only=True)
        if episode is None:
            print(f"Warning: Episode {i} is None, skipping...")
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Compute statistics for BSON Dex Dataset')
    ...
    if args.skip_exist and dataset_name in results:
        ...
        result = process_bson_dataset(vla_dataset)
